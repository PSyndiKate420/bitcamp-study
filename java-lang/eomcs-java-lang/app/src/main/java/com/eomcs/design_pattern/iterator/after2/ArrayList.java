...
public class ArrayList<E> {
  ...
  public Iterator<E> iterator() {
    /* 생성자 바깥 클래스의 인스턴스 주소를 넘겨주지 않아도
     * 컴파일러가 넘겨주는 코드로 자동 변환한다.*/
    return new ArrayListIterator<E>(/*this*/);
    /* 위 코드는 컴파일 될 때 다음과 같이 바뀐다. 
     * return new ArrayListIterator<E>(this)*/
  }

  class ArrayListIterator<T> implements Iterator<T> {

    /* 논스태틱 클래스인 경우 바깥 클래스의 인스턴스 주소를 받을 필드가
     * 자동 추가된다.
    ...
    public boolean hasNext() {
      /* 컴파일러가 자동으로 추가한
       * 바깥 클래스의 인스턴스 주소를 담고 있는 변수를 사용하려면
       * 다음과 같이 지정하라.
       * -> 바깥클래스명.this.멤버
       * */
    ...
    public T next() {
       ...
       * ArrayList.this라고 지정하면 된다.
       * ArrayList.get() 메소드의 리턴 타입이 E이다.
       * E가 가리키는 타입 정보는 결국 ArrayListIterator를 만들 때 넘겨준다.
       *  new ArrayListIterator<E>();
       * 따라서 ArrayListIterator 내부에서 사용하는 T가 곧 ArrayList의 E와 같다.
       * 그 결과 다음과 같이 형변환 할 수 있다.
       * */
